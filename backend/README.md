...
- **Índices**: `Project` tiene índices en `status` y `category` (filtros más habituales en listados y en las agregaciones de analytics); `AuditLog` tiene índice compuesto `{ entityType, entityId }` y otro en `createdAt` descendente (paginación por fecha, la vista por defecto del Audit Log). Con el volumen actual (cientos de documentos) el impacto es marginal, pero evita un table scan cuando la base de datos crezca.
- **Pendiente / roadmap**: introducir caché de servidor (Redis) si el número de usuarios concurrentes lo justifica — con el volumen actual (cientos de documentos, un puñado de usuarios) no aporta beneficio real; y pruebas de carga (k6/Artillery) antes de un eventual salto a tráfico concurrente real.
